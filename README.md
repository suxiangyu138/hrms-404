...
| `DB_PASSWORD` | **无默认值** | 数据库连接密码；**未设置则应用启动失败**（有意为之：宁可启动报错，也不要弱口令连库） |
...
> **这些口令只用于本地演示。** `admin` / `123456` 是公开写在文档里的，一旦部署到公网（或任何他人可访问的地址），任何人都能直接以系统管理员身份登录。
> 部署前务必：改掉 `admin` 密码、禁用或删除全部演示账号、清空 `sql/04_seed.sql` 写入的种子数据。
...
| 用户权限 | 会话登录 + MD5 加盐（盐 `404n0tf0und`，**仅教学用途**，见下）、RBAC 页面 + 接口双层校验、经理数据范围 = 部门子树 |
...
> **关于密码哈希：** `PasswordUtil` 用 `MD5(固定盐 + 密码)`，盐写死在源码里且全局唯一。
> 这样做的唯一理由是让**应用层加密与数据库触发器加密保持同一套算法**——种子数据里可以直接写死
> `MD5('404n0tf0und123456')` 构造出可登录的账号，不必绕一圈调 Java。
> 它挡不住彩虹表和同盐碰撞，**生产环境请换 BCrypt / Argon2**。
